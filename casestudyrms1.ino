...
int servo1PosSaves[] = {1,1,1,1,1}; //position saves initial values
...
void loop() {
  ...
  servo1.write(pot1Angle); // servos move to the mapped angles
  servo2.write(pot2Angle);
  servo3.write(pot3Angle);
  servo4.write(pot4Angle);
  
  if(digitalRead(button1) == HIGH){ // it checks how many times button1 is pressed and save the positions to an array depending on how many times it is pressed; 
//switch/case works like if statement
    button1Presses++;
...
if(button2Pressed){
  for(int i = 0; i < 5; i++){
    ...
    Serial.print("Potentiometer Angles: ");
    Serial.print(servo1PosSaves[i]);
    Serial.print(", ");
    Serial.print(servo2PosSaves[i]);
    Serial.print(", ");
    Serial.print(servo3PosSaves[i]);
    Serial.print(", ");
